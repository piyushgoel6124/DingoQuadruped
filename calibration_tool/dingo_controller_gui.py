#!/usr/bin/env python3
"""
Dingo Quadruped - Real-Time 3D Interactive Calibration & Control GUI
...
- PC Storage (dingo_config.json) holds persistent calibration configurations.
"""
...
# Physical dimensions for Dingo 3D Kinematics (in meters)
BODY_LENGTH = 0.28
BODY_WIDTH = 0.15
L_HIP = 0.045
L_UPPER = 0.095
...
class DingoGuiApp(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("Dingo Quadruped 3D Controller & Calibration Utility")
    ...
    def create_widgets(self):
        ...
        # PC Storage Controls
        pc_title = ttk.Label(ops_frame, text="PC Storage Controls (dingo_config.json):", font=("Consolas", 12, "bold"))
        ...
        self.plot_frame = ttk.LabelFrame(right_panel, text=" DINGO REAL-TIME KINEMATICS (3D WIREFRAME MODEL) ")
    ...
    def trigger_pose(self, pose_name):
        ...
        self.update_3d_view()
        print(f"Dingo commanded to predefined pose: {pose_name}")

    def relax_servos(self):
        if self.ser and self.ser.is_open:
            neutral_angles = [90.0] * 12
            self.push_all_angles(neutral_angles)
            print("Dingo servos set to neutral 90 degrees (R command not supported by updated Arduino).")
        else:
            messagebox.showwarning("Command Blocked", "Connect a live serial session before relaxing servos.")

    def save_to_pc(self):
        import json
        config_path = os.path.join(os.path.dirname(__file__), "dingo_config.json")
        try:
    ...
    def update_3d_view(self):
        ...
        self.ax.set_title("Dingo 3D Live Simulation View", color="#38bdf8", fontname="Consolas", fontsize=14, fontweight="bold")
