...
class RoboDogGazeboEnv(gym.Env):
    def __init__(self):
        super(RoboDogGazeboEnv, self).__init__()
        
        rospy.init_node('robodog_gym_env', anonymous=True)
        ...
        for name in self.joint_names:
            self.pub_joints[name] = rospy.Publisher(f'/robodog_controller/{name}/command', Float64, queue_with_latter=1)
            
        # Subscribers
        rospy.Subscriber('/robodog_gazebo/joint_states', JointState, self._joint_state_callback)
    ...
    def _model_states_callback(self, data):
        if 'robodog_gazebo' in data.name:
            idx = data.name.index('robodog_gazebo')
