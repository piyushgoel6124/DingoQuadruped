...
## Overview
This repository hosts the code for the RoboDog Quadruped, a robot designed to be low-cost but capable of conducting research and being extensively modified with additional actuators and sensors. CAD for the RoboDog can be found [here](https://github.com/Yerbert/RoboDogCAD). A full Bill of materials for purchasable components can be found within this repo.
...
The repository includes a driver node, robodog_driver.py, which should be used anytime the code is run. This file enables joystick control of the RoboDog, and allows joint and/or task space commands generated by any other code to be passed through the driver via the appropriate ROS command topics. The joystick can also be toggled on and off to override commands received via ROS command topics

The repo also includes a gazebo simulation of the RoboDog, based on URDF file and meshes which are also provide.

## How RoboDog_driver Works
...
<p align="center">
    <img src="assets/RoboDog_driver flow diagram.png" style="align:centre" width="50%">
</p>

## Project Structure
```.
├── assets                                    Images used in the readme file
├── robodog_nano                                Code for the Arduino Nano V3 to read sensor data and send it to the Raspberry Pi
└── robodog_ws                                  ROS workspace containing all required packages
   └── src
     ├── robodog                                Package containing node and launch files for running the robot
     ├── robodog_control                        Package containing all files related to control, including kinematics and default trot controller
     ├── robodog_description                    Package containing simulation files (URDF file and meshes)
     ├── robodog_gazebo                         Package containing gazebo files
     ├── robodog_hardware_interfacing
     |  ├── robodog_input_interfacing           Package containing files for receiving and interpreting commands (From a joystick or keyboard)
     |  ├── robodog_peripheral_interfacing      Package containing files for interfacing with the Arduino Nano, LCD screen and IMU
     |  └── robodog_servo_interfacing           Package containing the hardware interface for sending joint angles to the servo motors
     └── robodog_utilities                      Package containing useful utilities
```
...
- Clone this (the RoboDog Quadruped) repository using git: `git clone ...`
- Move into the robodog_ws folder: `cd /RoboDogQuadruped/robodog_ws`
- Initialise rosdep: `sudo rosdep init`
- Fetch dependencies with rosdep: `rosdep update`
- Build the workspace: `catkin build`
- Source the workspace: `source devel/setup.bash`
- (Optional) Add a line to .bashrc to automatically source the workspace: `echo "source ~/RoboDogQuadruped/robodog_ws/devel/setup.bash" >> ~/.bashrc`, `source ~/.bashrc`
...
- Run these commands to backup/restore. Replace source/destination appropriately.
    - Backup to file: `sudo dd if=/dev/sdb of=~/robodog_backup.img bs=4M status=progress`
    - Restore back to sdcard: `sudo dd if=robodog_backup of=/dev/sdb bs=4M status=progress`
...
Help with getting the servos calibrated
 ...
 - (robodog_hardware_interfacing/robodog_servo_interfacing/src/robodog_servo_interfacing/CalibrateServos.py)
 - Example commands:
    - `rosrun robodog_servo_interfacing CalibrateServos all cal` (move all servos to calibration position)
    - `rosrun robodog_servo_interfacing CalibrateServos fr high` (move front right servo to high position)
...
- In terminal, open the base folder containing the robodog quadruped code: `cd ~/any_folder_name/RoboDogQuadruped`
- run `code .` to open the robodog quadruped base folder in vscode
...
`roslaunch robodog robodog.launch is_physical:=0 is_sim:=1`

### RoboDog Joystick Controls
<p align="center">
    <img src="assets/joystick control map.jpg" style="align:centre" width="50%">
</p>

The diagram above shows the mapping of joystick commands for the RoboDog. These instructions are based on a PS4 type controller. An alternative, more general description of joystick commands is below:

The RoboDog has two main states: Rest and Trot. 
...
Make sure robodog_driver is running first, then:
`roslaunch robodog_gazebo simulation.launch`
